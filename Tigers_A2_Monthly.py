...
dftotal = pd.concat([pd.read_table(v, names = ['panid', 'date', 'kwh'], sep = " ", nrows = 2000000) for v in paths3])
times = pd.read_csv(time_data, usecols = [2,3,4,5,6,9,10], parse_dates = [0])
###Day light savings###--------------------------------------------------------
...
exl = pd.read_excel(main_dir + "\CER Electricity Data\CER Data\\" + excel_file, parse_cols = "A:E")
exl2 = exl[exl.Code == 3]
...
### Time Series
# Monthly AGGREGATION --------------------
df['ym'] = df['year'] + df['month']/100
grp = df.groupby(['ym', 'panid', 'tariff']) #was date
agg = grp['kwh'].sum()


# reset the index (multilevel at the moment)
agg = agg.reset_index() # drop the multi-index
grp = agg.groupby(['ym', 'tariff'])#was date

## split up treatment/control

trt = {k[0]: agg.kwh[v].values for k, v in grp.groups.iteritems() if k[1] == 'A'} # get set of all treatments by date (1 by day, 2 by month)
ctrl = {k[0]: agg.kwh[v].values for k, v in grp.groups.iteritems() if k[1] == 'E'} # get set of all controls by date (1 by day 2 by month)
keys = ctrl.keys()
...
p1.axvline(6, color = 'g', linestyle = '--')
plt.show()
p1.set_title("Monthly T Stats")

p2 = fig1.add_subplot(2,1,2)
p2.plot(t_p['pval']) 
p2.axhline(.05, color = 'r', linestyle = '--')
p2.axvline(6, color = 'g', linestyle = '--')
p2.set_title("Monthly P Values")